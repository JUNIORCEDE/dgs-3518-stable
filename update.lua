...
if not DGSConfig.enableUpdateSystem then
    return
end
            ...
            if not ManualUpdate then
                if RemoteVersion > version then
                    ...
                    updateTimer = setTimer(function()
                        if RemoteVersion > version then
                            outputDebugString("[DGS]Remote Version Got [Remote:" .. RemoteVersion .. " Current:" ..
                                                  version .. "].")
                            outputDebugString("[DGS]Update? Command: " .. DGSConfig.updateCommand)
                        else
                            killTimer(updateTimer)
    ...
    if getVersion().sortable < "1.5.4-9.11342" then
        if player then
            ...
            if acc and getAccountName(acc) == "Console" then
                ...
                outputChatBox("[DGS]Your server version is outdated upgrade to 1.5.4-9.11342 or higher", player, 255,
                    255, 0)
            end
            return false
        else
...
if DGSConfig.updateCheckAuto then
    if not checkServerVersion() then
        return
    end
    checkUpdate()
...
addCommandHandler(DGSConfig.updateCommand, function(player)
    if not checkServerVersion(player) then
        return
    end
...
function getGitHubTree(path, nextPath)
    ...
    fetchRemote(path or "https://github.com/JUNIORCEDE/dgs-3518-stable/tree/main", function(data, err)
        if err == 0 then
    ...
    for k, v in ipairs(xmlNodeGetChildren(xml)) do
        repeat
            if xmlNodeGetName(v) == "script" or xmlNodeGetName(v) == "file" then
                ...
                if string.find(path, "styleMapper.lua") then
                    break
                end
                if path == "meta.xml" then
                    break
                end
                if string.find(path, "test.lua") and not DGSConfig.enableTestFile then
                    break
                end
                local sha = ""
...
function DownloadFiles()
    ...
    outputDebugString("[DGS]Requesting (" .. UpdateCount .. "/" .. (#preUpdate or "Unknown") .. "): " ..
                          tostring(preUpdate[UpdateCount]) .. "")
    fetchRemote("https://raw.githubusercontent.com/thisdp/dgs/master/" .. preUpdate[UpdateCount],
        function(data, err, path)
            if err == 0 then
                ...
                fileClose(file)
                outputDebugString(
                    "[DGS]File Got (" .. UpdateCount .. "/" .. #preUpdate .. "): " .. path .. " [ " .. size .. "B -> " ..
                        newsize .. "B ]")
            else
...
function DownloadFinish()
    ...
    outputDebugString("[DGS]Update Complete ( " .. #preUpdate .. " File" .. (#preUpdate == 1 and "" or "s") ..
                          " Changed )")
    outputDebugString("[DGS]Please Restart DGS")
    outputChatBox("[DGS]Update Complete ( " .. #preUpdate .. " File" .. (#preUpdate == 1 and "" or "s") .. " Changed )",
        root, 0, 255, 0)
    preUpdate = {}
    UpdateCount = 0
end
...
addCommandHandler("dgsver", function(pla, cmd)
    ...
    if fileExists("update.cfg") then
        ...
        if vsdd then
            ...
            outputDebugString("[DGS]Version State is damaged! Please use /" .. DGSConfig.updateCommand .. " to update",
                1)
        end
    else
    ...
    if getPlayerName(pla) ~= "Console" then
        if vsdd then
            ...
            outputChatBox("[DGS]Version State is damaged! Please use /" .. DGSConfig.updateCommand .. " to update", pla,
                255, 0, 0)
        end
    end
end)
